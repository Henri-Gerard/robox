...
function proj_pi(f::Function,
    ...
    """
    Apply on vector x a projection step on function f.

    Parameters
    ----------
    f::Function
        The function on which we want to project.
    t::Array{Float64,1}
        Gradient of the function f.
    x::Array{Float64,1}
        Vector we want to project
    i::Int

    data::Array{Float64,2}

    regressionModel::RegressionModel
        Define the regression model used.
...
function x_in_inter(x::Array{Float64,1},
    ...
    Name of constraint
    """
    aux = @distributed (max) for k in 1:length(I)
...
function proj_In(xn::Array{Float64,1},
    ...
    """
    n::Int = length(I)
    ul::Array{Float64,1} = @distributed (+) for k in 1:n
...
function algo_proj_bis(x0::Array{Float64,1},
    ...
    while (iter < Int(round(length(robustModel.I0)/projParams.sample))+1) || ((dist_mem > projParams.precision) && (iter < projParams.ITER_MAX))
        ...
        push!(I, length(robustModel.I0)+1)
        ω = ones(length(I))/(length(I))
        res::Array{Float64,1} = @distributed (+) for k in 1:length(I)
